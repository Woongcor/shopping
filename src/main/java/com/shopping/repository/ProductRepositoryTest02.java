...
public class ProductRepositoryTest02 {
    ...
    @Test
    @DisplayName("특정 가격 이하 상품들만 조회하되, 가격에 대하여 내림차순 정렬")
    public void findByPriceLessThanOrderByPriceDescTest() {
        List<Product> itemList = ProductRepository
                .findByPriceLessThanOrderByPriceDesc(300);
        for (Product product : itemList) {
            System.out.println(product.toString());
        }
    }
    ...
    public void createProductTestMany() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            product.setPrice(price[i % fruit.length]);
            product.setDescription(description[i % fruit.length]);
            product.setProductStatus(ProductStatus.SELL);
            product.setStock(stock[i % fruit.length]);
    ...
    @Test
    @DisplayName("queryDslTest01")
    public void queryDslTest01() {
        // 현재 판매중인 상품 중에서, 상품 설명에 '아'자가 들어 있는 상품들 조회
        // 가격의 역순으로 출력합니다.
